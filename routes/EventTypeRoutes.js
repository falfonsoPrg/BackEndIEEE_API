...
router.delete("/:event_type_id", async (req, res) => {
  /**
        #swagger.tags = ['EventTypes']
        #swagger.path = '/eventtypes/{event_type_id}'
        #swagger.description = 'Endpoint to delete a eventtype'
        */
  const toDelete = {
    event_type_id: req.params.event_type_id,
  };

  const { error } = UpdateEventTypeValidation(toDelete);
  if (error)
    return res.status(422).send({
      error: error.details[0].message,
    });
  const eventtype = await EventTypeController.deleteEventType(toDelete);
  if (eventtype == 0) {
    return res.status(404).send({
      error: "Couldn't found that eventtype",
    });
